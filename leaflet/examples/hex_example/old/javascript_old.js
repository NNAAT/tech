...
function getHexColor(value) {
    return value > ((0.279 + 0.649) / 2) ? '#A6D96A':
	       value > ((0.072 + 0.279) / 2) ? '#FFFFBF':
	       value > ((0.000 + 0.072) / 2) ? '#FDAE61':
	                                       '#D7191C';           
}
...
function muniStyle(feature) {
	if (addMuniLegend == true) {
		legendMuni.addTo(map);
		addMuniLegend = false;
	}

	if (addMuniLegend == false) {
		legendMuni.addTo(map);
		addMuniLegend = true;
	}
...
function panelUpdate(props) {
	...
	if ((1 - props.v1_soil_polution_ratio) > 0) {
		var v1SoilPollution = '<br />' + 'V1 soil pollution ' + ((1 - props.v1_soil_polution_ratio) * 100).toFixed(2) + ' %';
	} else {
		var v1SoilPollution = '';
	}
	...
	if ((1 - props.forest_reserve_ratio) > 0) {
		var forestReserve = '<br />' + 'Forest reserve areas ' + ((1 - props.forest_reserve_ratio) * 100).toFixed(2) + ' %';
	} else {
		var forestReserve = '';
	}

	if ((1 - props.forest_ratio) > 0) {
		var forest = '<br />' + 'Forest ' + ((1 - props.forest_ratio) * 100).toFixed(2) + ' %';
	} else {
		var forest = '';
	}
	
	if ((1 - props.ramsar_area_ratio) > 0) {
		var ramsar = '<br />' + 'Ramsar areas ' + ((1 - props.ramsar_area_ratio) * 100).toFixed(2) + ' %';
	} else {
		var ramsar = '';
	}
...
*/
/*
geoJsonMuni.onAdd = function(map) {
	legendMuni.addTo(map);
};

geoJsonMuni.onRemove = function(map) {
	legendMuni.remove(map);
};
//	geoJsonMuni.removeFrom(map);
geoJsonHex.onAdd = function(map) {
	legendHex.addTo(map);
};

geoJsonHex.onRemove = function(map) {
//	geoJsonHex.removeFrom(map);
	legendHex.remove(map);
};
*/
/*
...
function getMarker(feature) {
	if (calculation(feature) > 0.6) {
		var marker = L.marker(features.geometry.coordinates).addTo(map); // SKAL KUN HAVE ET PUNKT UD
	}
		
	// HAVE THE LEGEND VALUES BEING GLOBAL
}
// value > 0.8 ? '#1A9641':
*/

/*
	if (map.hasLayer(geoJsonMuni) == false) {
		legendMuni.addTo(map);
	} else {
		legendMuni.remove(map);
	}
