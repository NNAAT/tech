...
CREATE MATERIALIZED VIEW MERGED_RESTRICTIVE_BUFFER
AS
(
...
(
    SELECT ROW_NUMBER() OVER ()::INT AS CPL_ID,
        ...
        FROM CHURCH_PROTECTION_LINE CPL,
            ...
            MERGED_RESTRICTIVE_BUFFER MVMRB
)
WITH DATA;
...
(
    SELECT ROW_NUMBER() OVER ()::INT AS LL_ID,
           ST_DIFFERENCE(ST_INTERSECTION(ST_BUFFER(LL.GEOM, 0.0001), MCL.GEOM), MVMRB.GEOM) AS GEOM
        FROM LOWLANDS LL,
            MUNICIPALITY_CUT_LAYER MCL,
            MERGED_RESTRICTIVE_BUFFER MVMRB
)
...
(
    SELECT ROW_NUMBER() OVER ()::INT AS PESD_ID,
        ...
        FROM PROTECTED_STONE_AND_EARTH_DIKES PESD,
            MUNICIPALITY_CUT_LAYER MCL,
            MERGED_RESTRICTIVE_BUFFER MVMRB
)
WITH DATA;
...
(
    SELECT ROW_NUMBER() OVER ()::INT AS DWI_ID,
           ST_DIFFERENCE(ST_INTERSECTION(ST_BUFFER(DWI.GEOM, 0.0001), ST_BUFFER(MCL.GEOM, 0.0001)), MVMRB.GEOM) AS GEOM
        FROM DRINKING_WATER_INTERESTS DWI,
            MUNICIPALITY_CUT_LAYER MCL,
            MERGED_RESTRICTIVE_BUFFER MVMRB
)
WITH DATA;
...
CREATE MATERIALIZED VIEW MV_WIND_SPEED_UNRECEPTIVE
AS
(
    SELECT ROW_NUMBER() OVER ()::INT AS CPL_ID,
           ST_DIFFERENCE(ST_INTERSECTION(WRL.GEOM, MCL.GEOM), MVMRB.GEOM) AS GEOM
        FROM WIND_RESOURCE_LAYER WRL,
            MUNICIPALITY_CUT_LAYER MCL,
            MERGED_RESTRICTIVE_BUFFER MVMRB
    WHERE (WRL.MEANW70 > 5.5 AND WRL.MEANW70 <= 6.5)
)
WITH DATA;

CREATE INDEX MV_GIX_WIND_SPEED_UNRECEPTIVE
ON MV_WIND_SPEED_UNRECEPTIVE
USING GIST (GEOM);


CREATE MATERIALIZED VIEW MV_WIND_SPEED_LIMITED
AS
(
    SELECT ROW_NUMBER() OVER ()::INT AS CPL_ID,
           ST_DIFFERENCE(ST_INTERSECTION(WRL.GEOM, MCL.GEOM), MVMRB.GEOM) AS GEOM
        FROM WIND_RESOURCE_LAYER WRL,
            MUNICIPALITY_CUT_LAYER MCL,
            MERGED_RESTRICTIVE_BUFFER MVMRB
    WHERE (WRL.MEANW70 > 6.5 AND WRL.MEANW70 <= 7.5)
)
WITH DATA;

CREATE INDEX MV_GIX_WIND_SPEED_LIMITED
ON MV_WIND_SPEED_LIMITED
USING GIST (GEOM);


CREATE MATERIALIZED VIEW MV_WIND_SPEED_ADVANTAGEOUS
AS
(
    SELECT ROW_NUMBER() OVER ()::INT AS CPL_ID,
           ST_DIFFERENCE(ST_INTERSECTION(WRL.GEOM, MCL.GEOM), MVMRB.GEOM) AS GEOM
        FROM WIND_RESOURCE_LAYER WRL,
            MUNICIPALITY_CUT_LAYER MCL,
            MERGED_RESTRICTIVE_BUFFER MVMRB
    WHERE (WRL.MEANW70 > 7.5)
)
WITH DATA;

CREATE INDEX MV_GIX_WIND_SPEED_ADVANTAGEOUS
ON MV_WIND_SPEED_ADVANTAGEOUS
USING GIST (GEOM);
...
-- Calculate available area per grid cell
ALTER TABLE HEXGRID_150
ADD COLUMN AVAILABLE_AREA
NUMERIC NOT NULL DEFAULT 1;

UPDATE HEXGRID_150
SET AVAILABLE_AREA = SHAPE_AREA;

UPDATE HEXGRID_150
    SET AVAILABLE_AREA = IQ.AREA
    FROM (SELECT HG.GID ID,
                ROUND(CAST(FLOAT8(HG.SHAPE_AREA - ST_AREA(ST_UNION(ST_INTERSECTION(MRB.GEOM, HG.GEOM)))) AS NUMERIC), 6) AREA
        FROM HEXGRID_150 HG
            JOIN MERGED_RESTRICTIVE_BUFFER MRB
                ON ST_INTERSECTS(HG.GEOM, MRB.GEOM)
        GROUP BY HG.GID) AS IQ
WHERE HEXGRID_150.GID = IQ.ID;


-- Calculate what is the cell availability within the restrictive layers
...
UPDATE HEXGRID_150
SET WIND_TURBINE_SITE_RATIO = 0
WHERE AVAILABLE_AREA = 0;

UPDATE HEXGRID_150
    SET WIND_TURBINE_SITE_RATIO = IQ.PRCNT
    FROM (SELECT HG.GID ID,
                ROUND(CAST(FLOAT8(ST_AREA(ST_UNION(ST_INTERSECTION(MVWTSB.GEOM, HG.GEOM))) / HG.AVAILABLE_AREA) AS NUMERIC), 6) PRCNT
        FROM HEXGRID_150 HG,
             MV_WIND_TURBINE_SITE_BUFFER MVWTSB
        WHERE HG.AVAILABLE_AREA > 0
            AND ST_INTERSECTS(HG.GEOM, MVWTSB.GEOM)
        GROUP BY HG.GID) AS IQ
WHERE HEXGRID_150.GID = IQ.ID;
...
UPDATE HEXGRID_150
SET MUNICIPALITY_ROAD_RATIO = 0
WHERE AVAILABLE_AREA = 0;

UPDATE HEXGRID_150
    SET MUNICIPALITY_ROAD_RATIO = IQ.PRCNT
    FROM (SELECT HG.GID ID,
                ROUND(CAST(FLOAT8(ST_AREA(ST_UNION(ST_INTERSECTION(ST_BUFFER(MVMRB.GEOM, 0.000001), ST_FORCE2D(HG.GEOM)))) / HG.AVAILABLE_AREA) AS NUMERIC), 6) PRCNT
        FROM HEXGRID_150 HG,
            MV_MUNICIPALITY_ROAD_BUFFER MVMRB
        WHERE HG.AVAILABLE_AREA > 0
            AND ST_INTERSECTS(HG.GEOM, MVMRB.GEOM)
        GROUP BY HG.GID) AS IQ
WHERE HEXGRID_150.GID = IQ.ID;
...
UPDATE HEXGRID_150
SET MUNICIPALITY_ROAD_RATIO = 0
WHERE MUNICIPALITY_ROAD_RATIO > 1.04;

UPDATE HEXGRID_150
SET MUNICIPALITY_ROAD_RATIO = 1
WHERE MUNICIPALITY_ROAD_RATIO > 1;

--                          OTHER LAYERS                          --
--------------------------------------------------------------------

-- Calculate restrictive layer evals
...
UPDATE HEXGRID_150
    SET BEACH_PROTECTION_RATIO = 0
WHERE AVAILABLE_AREA = 0;

UPDATE HEXGRID_150
    SET BEACH_PROTECTION_RATIO = IQ.PRCNT
    FROM (SELECT HG.GID ID,
            ROUND(CAST(FLOAT8(ST_AREA(ST_UNION(ST_INTERSECTION(MVBP.GEOM, HG.GEOM))) / HG.AVAILABLE_AREA) AS NUMERIC), 6) PRCNT
        FROM HEXGRID_150 HG,
            MV_BEACH_PROTECTION MVBP
        WHERE HG.AVAILABLE_AREA > 0
            AND ST_INTERSECTS(HG.GEOM, MVBP.GEOM)
        GROUP BY HG.GID) AS IQ
WHERE HEXGRID_150.GID = IQ.ID;

UPDATE HEXGRID_150
    SET BEACH_PROTECTION_RATIO = 1
WHERE BEACH_PROTECTION_RATIO > 1;
...
UPDATE HEXGRID_150
    SET CHURCH_PROTECTION_LINE_RATIO = 0
WHERE AVAILABLE_AREA = 0;

UPDATE HEXGRID_150
    SET CHURCH_PROTECTION_LINE_RATIO = IQ.PRCNT
    FROM (SELECT HG.GID ID,
            ROUND(CAST(FLOAT8(ST_AREA(ST_UNION(ST_INTERSECTION(MVCPL.GEOM, HG.GEOM))) / HG.AVAILABLE_AREA) AS NUMERIC), 6) PRCNT
        FROM HEXGRID_150 HG,
            MV_CHURCH_PROTECTION_LINE MVCPL
        WHERE HG.AVAILABLE_AREA > 0
            AND ST_INTERSECTS(HG.GEOM, MVCPL.GEOM)
        GROUP BY HG.GID) AS IQ
WHERE HEXGRID_150.GID = IQ.ID;
...
UPDATE HEXGRID_150
    SET COAST_LINE_ZONE_RATIO = 0
WHERE AVAILABLE_AREA = 0;

UPDATE HEXGRID_150
    SET COAST_LINE_ZONE_RATIO = IQ.PRCNT
    FROM (SELECT HG.GID ID,
            ROUND(CAST(FLOAT8(ST_AREA(ST_UNION(ST_INTERSECTION(MVCLZ.GEOM, HG.GEOM))) / HG.AVAILABLE_AREA) AS NUMERIC), 6) PRCNT
        FROM HEXGRID_150 HG
...
UPDATE HEXGRID_150
    SET COAST_LINE_ZONE_RATIO = 1
WHERE COAST_LINE_ZONE_RATIO > 1;
...
UPDATE HEXGRID_150
    SET LAKE_PROTECTION_LINE_RATIO = 0
WHERE AVAILABLE_AREA = 0;

UPDATE HEXGRID_150
    SET LAKE_PROTECTION_LINE_RATIO = IQ.PRCNT
    FROM (SELECT HG.GID ID,
            ROUND(CAST(FLOAT8(ST_AREA(ST_UNION(ST_INTERSECTION(MVLPL.GEOM, HG.GEOM))) / HG.AVAILABLE_AREA) AS NUMERIC), 6) PRCNT
        FROM HEXGRID_150 HG,
            MV_LAKE_PROTECTION_LINE MVLPL
        WHERE HG.AVAILABLE_AREA > 0
            AND ST_INTERSECTS(HG.GEOM, MVLPL.GEOM)
        GROUP BY HG.GID) AS IQ
WHERE HEXGRID_150.GID = IQ.ID;

UPDATE HEXGRID_150
    SET LAKE_PROTECTION_LINE_RATIO = 1
WHERE LAKE_PROTECTION_LINE_RATIO > 1;
...
UPDATE HEXGRID_150
    SET LOWLANDS_RATIO = 0
WHERE AVAILABLE_AREA = 0;

UPDATE HEXGRID_150
    SET LOWLANDS_RATIO = IQ.PRCNT
    FROM (SELECT HG.GID ID,
            ROUND(CAST(FLOAT8(ST_AREA(ST_UNION(ST_BUFFER(MV_LOWLANDS.GEOM, 0.0001), ST_FORCE2D(HG.GEOM))) / HG.AVAILABLE_AREA) AS NUMERIC), 6) PRCNT
        FROM HEXGRID_150 HG,
            MV_LOWLANDS
        WHERE HG.AVAILABLE_AREA > 0
            AND ST_INTERSECTS(HG.GEOM, MV_LOWLANDS.GEOM)
        GROUP BY HG.GID) AS IQ
WHERE HEXGRID_150.GID = IQ.ID;

UPDATE HEXGRID_150
    SET LOWLANDS_RATIO = 1
WHERE LOWLANDS_RATIO > 1.000030;

UPDATE HEXGRID_150
    SET LOWLANDS_RATIO = 1
WHERE LOWLANDS_RATIO > 1;
...
UPDATE HEXGRID_150
    SET PROTECTED_EARTH_STONE_DIKES_RATIO = 0
WHERE AVAILABLE_AREA = 0;

UPDATE HEXGRID_150
    SET PROTECTED_EARTH_STONE_DIKES_RATIO = IQ.PRCNT
    FROM (SELECT HG.GID ID,
            ROUND(CAST(FLOAT8(ST_AREA(ST_UNION(ST_INTERSECTION(MVPSED.GEOM, HG.GEOM))) / HG.AVAILABLE_AREA) AS NUMERIC), 6) PRCNT
        FROM HEXGRID_150 HG,
            MV_PROTECTED_EARTH_STONE_DIKES MVPSED
        WHERE HG.AVAILABLE_AREA > 0
            AND ST_INTERSECTS(HG.GEOM, MVPSED.GEOM)
        GROUP BY HG.GID) AS IQ
WHERE HEXGRID_150.GID = IQ.ID;
...
UPDATE HEXGRID_150
    SET RAW_MATERIAL_SITES_RATIO = 0
WHERE AVAILABLE_AREA = 0;

UPDATE HEXGRID_150
    SET RAW_MATERIAL_SITES_RATIO = IQ.PRCNT
    FROM (SELECT HG.GID ID,
           ROUND(CAST(FLOAT8(ST_AREA(ST_UNION(ST_INTERSECTION(MVRMS.GEOM, HG.GEOM))) / HG.AVAILABLE_AREA) AS NUMERIC), 6) PRCNT
        FROM HEXGRID_150 HG,
            MV_RAW_MATERIAL_SITES MVRMS
        WHERE HG.AVAILABLE_AREA > 0
            AND ST_INTERSECTS(HG.GEOM, MVRMS.GEOM)
        GROUP BY HG.GID) AS IQ
WHERE HEXGRID_150.GID = IQ.ID;
...
UPDATE HEXGRID_150
    SET STREAM_PROTECTION_LINE_RATIO = 0
WHERE AVAILABLE_AREA = 0;

UPDATE HEXGRID_150
    SET STREAM_PROTECTION_LINE_RATIO = IQ.PRCNT
    FROM (SELECT HG.GID ID,
            ROUND(CAST(FLOAT8(ST_AREA(ST_UNION(ST_INTERSECTION(MVSPL.GEOM, HG.GEOM))) / HG.AVAILABLE_AREA) AS NUMERIC), 6) PRCNT
        FROM HEXGRID_150 HG,
            MV_STREAM_PROTECTION_LINE MVSPL
        WHERE HG.AVAILABLE_AREA > 0
            AND ST_INTERSECTS(HG.GEOM, MVSPL.GEOM)
        GROUP BY HG.GID) AS IQ
WHERE HEXGRID_150.GID = IQ.ID;

UPDATE HEXGRID_150
    SET STREAM_PROTECTION_LINE_RATIO = 1
WHERE STREAM_PROTECTION_LINE_RATIO > 1;  
...
UPDATE HEXGRID_150
    SET TECHNICAL_AREAS_RATIO = 0
WHERE AVAILABLE_AREA = 0;

UPDATE HEXGRID_150
    SET TECHNICAL_AREAS_RATIO = IQ.PRCNT
    FROM (SELECT HG.GID ID,
            ROUND(CAST(FLOAT8(ST_AREA(ST_UNION(ST_INTERSECTION(MVTA.GEOM, HG.GEOM))) / HG.AVAILABLE_AREA) AS NUMERIC), 6) PRCNT
        FROM HEXGRID_150 HG,
            MV_TECHNICAL_AREAS MVTA
        WHERE HG.AVAILABLE_AREA > 0
            AND ST_INTERSECTS(HG.GEOM, MVTA.GEOM)
        GROUP BY HG.GID) AS IQ
WHERE HEXGRID_150.GID = IQ.ID;
...
UPDATE HEXGRID_150
    SET V1_SOIL_POLUTION_RATIO = 0
WHERE AVAILABLE_AREA = 0;

UPDATE HEXGRID_150
    SET V1_SOIL_POLUTION_RATIO = IQ.PRCNT
    FROM (SELECT HG.GID ID,
            ROUND(CAST(FLOAT8(ST_AREA(ST_UNION(ST_INTERSECTION(MVSP.GEOM, HG.GEOM))) / HG.AVAILABLE_AREA) AS NUMERIC), 6) PRCNT
        FROM HEXGRID_150 HG,
            MV_V1_SOIL_POLUTION MVSP
        WHERE HG.AVAILABLE_AREA > 0
            AND ST_INTERSECTS(HG.GEOM, MVSP.GEOM)
        GROUP BY HG.GID) AS IQ
WHERE HEXGRID_150.GID = IQ.ID;
...
UPDATE HEXGRID_150
    SET WETLANDS_RATIO = 0
WHERE AVAILABLE_AREA = 0;

UPDATE HEXGRID_150
    SET WETLANDS_RATIO = IQ.PRCNT
    FROM (SELECT HG.GID ID,
            ROUND(CAST(FLOAT8(ST_AREA(ST_UNION(ST_INTERSECTION(MV_WETLANDS.GEOM, HG.GEOM))) / HG.AVAILABLE_AREA) AS NUMERIC), 6) PRCNT
        FROM HEXGRID_150 HG,
            MV_WETLANDS
        WHERE HG.AVAILABLE_AREA > 0
            AND ST_INTERSECTS(HG.GEOM, MV_WETLANDS.GEOM)
        GROUP BY HG.GID) AS IQ
WHERE HEXGRID_150.GID = IQ.ID;

UPDATE HEXGRID_150
    SET WETLANDS_RATIO = 1
WHERE WETLANDS_RATIO > 1;
...
UPDATE HEXGRID_150
    SET FOREST_PROTECTION_LINE_RATIO = 0
WHERE AVAILABLE_AREA = 0;

UPDATE HEXGRID_150
    SET FOREST_PROTECTION_LINE_RATIO = IQ.PRCNT
    FROM (SELECT HG.GID ID,
            ROUND(CAST(FLOAT8(ST_AREA(ST_UNION(ST_INTERSECTION(MVFPL.GEOM, HG.GEOM))) / HG.AVAILABLE_AREA) AS NUMERIC), 6) PRCNT
        FROM HEXGRID_150 HG,
            MV_FOREST_PROTECTION_LINE MVFPL
        WHERE HG.AVAILABLE_AREA > 0
            AND ST_INTERSECTS(HG.GEOM, MVFPL.GEOM)
        GROUP BY HG.GID) AS IQ
WHERE HEXGRID_150.GID = IQ.ID;

UPDATE HEXGRID_150
    SET FOREST_PROTECTION_LINE_RATIO = 1
WHERE FOREST_PROTECTION_LINE_RATIO > 1;
...
UPDATE HEXGRID_150
    SET DISTRICT_PLAN_RATIO = 0
WHERE AVAILABLE_AREA = 0;

UPDATE HEXGRID_150
    SET DISTRICT_PLAN_RATIO = IQ.PRCNT
    FROM (SELECT HG.GID ID,
        ROUND(CAST(FLOAT8(ST_AREA(ST_UNION(ST_INTERSECTION(MVDP.GEOM, HG.GEOM))) / HG.AVAILABLE_AREA) AS NUMERIC), 6) PRCNT
        FROM HEXGRID_150 HG,
            MV_DISTRICT_PLAN MVDP
        WHERE HG.AVAILABLE_AREA > 0
            AND ST_INTERSECTS(HG.GEOM, MVDP.GEOM)
        GROUP BY HG.GID) AS IQ
WHERE HEXGRID_150.GID = IQ.ID;
...
UPDATE HEXGRID_150
    SET DRINKING_WATER_INTEREST_RATIO = 0
WHERE AVAILABLE_AREA = 0;

UPDATE HEXGRID_150
    SET DRINKING_WATER_INTEREST_RATIO = IQ.PRCNT
    FROM (SELECT HG.GID ID,
        ROUND(CAST(FLOAT8(ST_AREA(ST_UNION(ST_INTERSECTION(MVDWI.GEOM, ST_FORCE2D(HG.GEOM)))) / HG.AVAILABLE_AREA) AS NUMERIC), 6) PRCNT
        FROM HEXGRID_150 HG,
            MV_DRINKING_WATER_INTEREST MVDWI
        WHERE HG.AVAILABLE_AREA > 0
            AND ST_INTERSECTS(HG.GEOM, MVDWI.GEOM)
        GROUP BY HG.GID) AS IQ
WHERE HEXGRID_150.GID = IQ.ID;

UPDATE HEXGRID_150
SET DRINKING_WATER_INTEREST_RATIO = 0
WHERE DRINKING_WATER_INTEREST_RATIO > 1
AND AVAILABLE_AREA < 1;


ALTER TABLE HEXGRID_150
ADD COLUMN WIND_SPEED_UNRECEPTIVE_RATIO
NUMERIC NOT NULL DEFAULT 1;

UPDATE HEXGRID_150
    SET WIND_SPEED_UNRECEPTIVE_RATIO = 0
WHERE AVAILABLE_AREA = 0;

UPDATE HEXGRID_150
    SET WIND_SPEED_UNRECEPTIVE_RATIO = IQ.PRCNT
    FROM (SELECT HG.GID ID,
        ROUND(CAST(FLOAT8(ST_AREA(ST_UNION(ST_INTERSECTION(MVWSPU.GEOM, HG.GEOM))) / HG.AVAILABLE_AREA) AS NUMERIC), 6) PRCNT
        FROM HEXGRID_150 HG,
            MV_WIND_SPEED_UNRECEPTIVE MVWSPU
        WHERE HG.AVAILABLE_AREA > 0
            AND ST_INTERSECTS(HG.GEOM, MVWSPU.GEOM)
        GROUP BY HG.GID) AS IQ
WHERE HEXGRID_150.GID = IQ.ID;

UPDATE HEXGRID_150
    SET WIND_SPEED_UNRECEPTIVE_RATIO = 1
WHERE WIND_SPEED_UNRECEPTIVE_RATIO > 1;


ALTER TABLE HEXGRID_150
ADD COLUMN WIND_SPEED_LIMITED_RATIO
NUMERIC NOT NULL DEFAULT 1;

UPDATE HEXGRID_150
    SET WIND_SPEED_LIMITED_RATIO = 0
WHERE AVAILABLE_AREA = 0;

UPDATE HEXGRID_150
    SET WIND_SPEED_LIMITED_RATIO = IQ.PRCNT
    FROM (SELECT HG.GID ID,
        ROUND(CAST(FLOAT8(ST_AREA(ST_UNION(ST_INTERSECTION(MVWSPL.GEOM, HG.GEOM))) / HG.AVAILABLE_AREA) AS NUMERIC), 6) PRCNT
        FROM HEXGRID_150 HG,
            MV_WIND_SPEED_LIMITED MVWSPL
        WHERE HG.AVAILABLE_AREA > 0
            AND ST_INTERSECTS(HG.GEOM, MVWSPL.GEOM)
        GROUP BY HG.GID) AS IQ
WHERE HEXGRID_150.GID = IQ.ID;

UPDATE HEXGRID_150
    SET WIND_SPEED_LIMITED_RATIO = 0
WHERE WIND_SPEED_LIMITED_RATIO > 1;


ALTER TABLE HEXGRID_150
ADD COLUMN WIND_SPEED_ADVANTAGEOUS_RATIO
NUMERIC NOT NULL DEFAULT 1;

UPDATE HEXGRID_150
    SET WIND_SPEED_ADVANTAGEOUS_RATIO = 0
WHERE AVAILABLE_AREA = 0;

UPDATE HEXGRID_150
    SET WIND_SPEED_ADVANTAGEOUS_RATIO = IQ.PRCNT
    FROM (SELECT HG.GID ID,
        ROUND(CAST(FLOAT8(ST_AREA(ST_UNION(ST_INTERSECTION(MVWSPA.GEOM, HG.GEOM))) / HG.AVAILABLE_AREA) AS NUMERIC), 6) PRCNT
        FROM HEXGRID_150 HG,
            MV_WIND_SPEED_ADVANTAGEOUS MVWSPA
        WHERE HG.AVAILABLE_AREA > 0
            AND ST_INTERSECTS(HG.GEOM, MVWSPA.GEOM)
        GROUP BY HG.GID) AS IQ
WHERE HEXGRID_150.GID = IQ.ID;

UPDATE HEXGRID_150
    SET WIND_SPEED_ADVANTAGEOUS_RATIO = 0
WHERE WIND_SPEED_ADVANTAGEOUS_RATIO > 1;




-- Data corrections for layer ratios, fixing coverage issue when the layer doesn't cover
-- the area which is not restricted
UPDATE HEXGRID_150
    SET WIND_TURBINE_SITE_RATIO = 0
WHERE GID NOT IN
(
    SELECT HG.GID
    FROM HEXGRID_150 HG
        JOIN MV_WIND_TURBINE_SITE_BUFFER MVWTSB
            ON ST_INTERSECTS(HG.GEOM, MVWTSB.GEOM)
);
...
(
    SELECT HG.GID
    FROM HEXGRID_150 HG
        JOIN MV_MUNICIPALITY_ROAD_BUFFER MVMRB
            ON ST_INTERSECTS(HG.GEOM, MVMRB.GEOM)
);
...
(
    SELECT HG.GID
    FROM HEXGRID_150 HG
        JOIN MV_BEACH_PROTECTION MVBP
            ON ST_INTERSECTS(HG.GEOM, MVBP.GEOM)
);
...
(
    SELECT HG.GID
    FROM HEXGRID_150 HG
        JOIN MV_PROTECTED_EARTH_STONE_DIKES MVPSED
...
(
    SELECT HG.GID
    FROM HEXGRID_150 HG
        JOIN MV_DRINKING_WATER_INTEREST MVDWI
...
(
    SELECT HG.GID
    FROM HEXGRID_150 HG
        JOIN MV_WIND_SPEED_UNRECEPTIVE MVWSPU
            ON ST_INTERSECTS(HG.GEOM, MVWSPU.GEOM)
);
...
(
    SELECT HG.GID
    FROM HEXGRID_150 HG
        JOIN MV_WIND_SPEED_LIMITED MVWSPL
            ON ST_INTERSECTS(HG.GEOM, MVWSPL.GEOM)
);
...
(
    SELECT HG.GID
    FROM HEXGRID_150 HG
        JOIN MV_WIND_SPEED_ADVANTAGEOUS MVWSPA
            ON ST_INTERSECTS(HG.GEOM, MVWSPA.GEOM)
);